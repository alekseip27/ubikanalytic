    ...
    request.onload = function() {
        ...
        if (request.status >= 200 && request.status < 400) {
            ...
            data.results.forEach(events => {
                ...
                if (evid.startsWith("tm")) {
                card.setAttribute('eventid', evidp);
                }
                ...
                // NEW helper function that re-fetches updated counts (using the fresh event data)
                function updateChartWithPrimaryAndPreferredForEvent(eventData) {
                    let amountsPrimary = [];
                    let datesPrimary = [];
                    let combinedDates = new Set();
                    let preferredData = [];
                    let counts = eventData.counts; // using fresh data
                    let source = eventData.scraper_name.toLowerCase();
                    let venueid = eventData.site_event_id;
