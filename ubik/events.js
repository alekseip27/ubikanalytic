...
document.getElementById('rightarrow').addEventListener('click', function() {
  ...
  document.querySelector('#vserror').style.display = 'flex';
        ...
        async function getchartprimary() {
            ...
            .then(data => {
                ...
                console.log("Counts Length:", counts.length); // Check the length of counts
                if (counts.length <= 1) {
                    console.log("Only one or no data points to display.");
                }
  ...
  let app_142_scrape_date = '2024-04-06';
  let today = moment().tz('America/New_York').format('YYYY-MM-DD');

  let scrapeDate = moment(app_142_scrape_date);
  let currentDate = moment(today);
  
   difference = scrapeDate.diff(currentDate, 'days');
